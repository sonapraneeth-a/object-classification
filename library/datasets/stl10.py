...
class STL10:
    ...
    def __init__(self, num_images=1.0, one_hot_encode=False, train_validate_split=None,
                 num_test_images=1.0, endian='big', verbose=False):
        ...
        self.file_url = 'http://ai.stanford.edu/~acoates/stl10/stl10_binary.tar.gz'
    ...
    def download_and_extract_data(self, data_directory):
        print('Downloading and extracting STL 10 file')
        ## Step 1: Make the directories './datasets/STL10/' if they do not exist
        ...
        ## Step 2: Check if './datasets/STL10/STL-10-python.tar.gz' exists
        tar_file = data_directory + 'STL-10.tar.gz'
        make_tar = False
        if not os.path.exists(tar_file):
            ...
            if self.verbose is True:
                print('STL 10 tarfile exists and MD5 sum is verified')
        ## Step 3: Download STL 10 dataset from 'http://www.cs.toronto.edu/~kriz/STL-10-python.tar.gz'
        if make_tar is True:
            result = file_utils.download(self.file_url, tar_file, verbose=self.verbose)
            if result is False:
                if self.verbose is True:
                    raise FileNotFoundError('Download of STL 10 dataset failed')
                return False
            result = file_utils.verify_md5(tar_file, self.file_md5, verbose=self.verbose)
            if result is False:
                if self.verbose is True:
                    raise FileNotFoundError('Downloaded STL 10 dataset failed md5sum check')
                return False
        ## Step 4: Extract the datas
        make_extract = False
        batches_directory = data_directory + 'STL-10-batches'
        ...
        if make_extract is True:
            ...
            shutil.move('./STL-10-batches-py', batches_directory)
            if result is False:
                if self.verbose is True:
                    print('Extraction of STL 10 dataset failed')
                return False
            else:
                if self.verbose is True:
                    print('Extraction of STL 10 dataset success')
        return True
    ...
    def convert_one_hot_encoding(self, classes, data_type='train'):
        ...
        if data_type == 'train':
            self.train.labels = np.zeros((classes.shape[0], num_classes))
        if data_type == 'validate':
            self.validate.labels = np.zeros((classes.shape[0], num_classes))
        if data_type == 'test':
            self.test.labels = np.zeros((classes.shape[0], num_classes))
        for i in range(classes.shape[0]):
            if self.endian == 'big':
                if data_type == 'train':
                    self.train.labels[i, num_classes - 1 - classes[i]] = 1
                if data_type == 'validate':
                    self.validate.labels[i, num_classes - 1 - classes[i]] = 1
                if data_type == 'test':
                    self.test.labels[i, num_classes-1-classes[i]] = 1
            if self.endian == 'little':
                if data_type == 'train':
                    self.train.labels[i, classes[i]] = 1
                if data_type == 'validate':
                    self.validate.labels[i, classes[i]] = 1
                if data_type == 'test':
                    self.test.labels[i, classes[i]] = 1
    ...
    def load_data(self, train=True, test=True, data_directory='/tmp/STL10/'):
        ...
        print('Loaded STL 10 Dataset in %.4f seconds' %(end-start))
        return True
    ...
    def plot_sample(self, plot_data=True, plot_test=False, verbose=False, fig_size=(7, 7), fontsize=12,
        ...
        if plot_data is True:
            print('Plotting STL 10 Train Dataset')
            data_fig = plt.figure()
        ...
        if plot_test is True:
            print('Plotting STL 10 Test Dataset')
            test_fig = plt.figure()
    ...
    def plot_images(self, images, cls_true, cls_pred=None):
        assert images.shape[0] == cls_true.shape[0] == 9
